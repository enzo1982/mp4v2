...
static void sync_duration( char *toFileName, MP4FileHandle durfile )
{
    MP4Duration todur;
    ...
    tofile = MP4Create( newname.c_str() );
    ...
    MP4Close( tofile );
    MP4Close( fromfile );
    io::FileSystem::rename( newname, toFileName );
}
...
extern "C" int main( int argc, char** argv )
{
    ...
    char* difflist;
    char Mp4FileName[MP4V2_PATH_MAX], toFileName[MP4V2_PATH_MAX];
#if 0
    ...
    while ( true ) {
        ...
        static const prog::Option long_options[] = {
            ...
            { "force-meta", prog::Option::NO_ARG,       0, 'f' },
            { NULL, prog::Option::NO_ARG, 0, 0 }
        };

        c = prog::getOptionSingle( argc, argv, "v::V", long_options, &option_index );
    ...
    while ( prog::optind < argc ) {
        difflist = argv[prog::optind++];
        FILE *lfile = fopen( difflist, "r" );
        while ( fgets( Mp4FileName, MP4V2_PATH_MAX, lfile ) != NULL ) {
            ...
            if ( !mp4File ) {
                ...
                bool found = io::FileSystem::isFile( toFileName );
                if ( !found ) {
                    ...
                    for ( uint32_t ix = 1; ix < 36 && found == false; ix++ ) {
                        lastslash[0] = ( ix / 10 ) + '0';
                        lastslash[1] = ( ix % 10 ) + '0';
                        found = io::FileSystem::isFile( toFileName );
                    }
                }
                if ( found == false ) {
